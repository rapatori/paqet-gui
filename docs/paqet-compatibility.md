...
| `server.addr`                | address string       | Required; port `1–65535`; address family must have matching network configuration                                | Generated from selected profile                                             |
...
| `transport.tcpbuf`           | integer bytes        | `0` becomes `8192`; values below `4096` are clamped to `4096`; no upper bound in tagged validation               | Optional Advanced override, minimum `4096`                                  |
| `transport.udpbuf`           | integer bytes        | `0` becomes `4096`; values below `2048` are clamped to `2048`; no upper bound in tagged validation               | Optional Advanced override, minimum `2048`                                  |
